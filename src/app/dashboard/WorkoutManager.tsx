...
import { fetchSmartItemsAction } from '@/actions/treinus';
import { useSmartItemsStore } from '@/store/useSmartItemsStore';
import { useRouter } from 'next/navigation';
import React from 'react';

export function WorkoutManager() {
  const { smartItems, setSmartItems, clearSmartItems, lastUpdate } =
    useSmartItemsStore();
  ...
  const handleUpdate = () => {
    startTransition(async () => {
      const result = await fetchSmartItemsAction();

      if (result.success) {
        setSmartItems(result.data);
      } else {
        clearSmartItems();
        router.push('/login');
      }
        ...
        {smartItems ? (
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs font-mono text-zinc-500 dark:text-zinc-500">
              Payload: {(JSON.stringify(smartItems).length / 1024).toFixed(2)}{' '}
              KB
            </p>
          </div>
        ) : (
